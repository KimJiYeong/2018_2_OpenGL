...
void SetupRC()
{
	for (int i = 0; i < 5; i++) {
		...
		piramid[i].index[0].x = 0;
		piramid[i].index[0].y = 0;
		piramid[i].index[0].z = 0;

		piramid[i].index[1].x = 0;
		piramid[i].index[1].y = 0;
		piramid[i].index[1].z = 1;

		piramid[i].index[2].x = 1;
		piramid[i].index[2].y = 0;
		piramid[i].index[2].z = 1;

		piramid[i].index[3].x = 1;
		piramid[i].index[3].y = 0;
		piramid[i].index[3].z = 0;

		piramid[i].index[4].x = 0.5;
		piramid[i].index[4].y = 1;
		piramid[i].index[4].z = 0.5;

		piramid[i].scale.x = 20;
		piramid[i].scale.y = 20;
		piramid[i].scale.z = 20;
	}
	...
	piramid[1].pos.x = Ground_size;
	piramid[1].pos.z = -Ground_size;

	piramid[2].pos.x = -Ground_size;
	piramid[2].pos.z = -Ground_size;

	piramid[3].pos.x = -Ground_size;
	piramid[3].pos.z = Ground_size;

	piramid[4].pos.x = Ground_size;
	piramid[4].pos.z = Ground_size;


	moon.pos.x = piramid[0].pos.x;
	moon.pos.y = lightPos[1];
	moon.pos.z = piramid[0].pos.z;
...
GLvoid drawScene(GLvoid)
{
	...
	glPushMatrix();//-----------------------------------
	{
		...
		glPushMatrix();//-------------그리기 입력--------------------------
		{
			...
			glPushMatrix();//달
			{
				glTranslated(moon.pos.x, moon.pos.y, moon.pos.z);
				glutSolidSphere(30, 8, 8);//원을 만든다.
			}
			glPopMatrix();//달
				...
				for (int i = 0; i < 5; i++) {
					glPushMatrix();//피라미드
					{
						glTranslated(piramid[i].pos.x, piramid[i].pos.y, piramid[i].pos.z);
						glScalef(piramid[i].scale.x, piramid[i].scale.y, piramid[i].scale.z);

						glBegin(GL_POLYGON);
						glVertex3d(piramid[i].index[0].x, piramid[i].index[0].y, piramid[i].index[0].z);
						glVertex3d(piramid[i].index[1].x, piramid[i].index[1].y, piramid[i].index[1].z);
						glVertex3d(piramid[i].index[2].x, piramid[i].index[2].y, piramid[i].index[2].z);
						glVertex3d(piramid[i].index[3].x, piramid[i].index[3].y, piramid[i].index[3].z);
						glEnd();

						glBegin(GL_TRIANGLES);
						glVertex3d(piramid[i].index[1].x, piramid[i].index[1].y, piramid[i].index[1].z);
						glVertex3d(piramid[i].index[2].x, piramid[i].index[2].y, piramid[i].index[2].z);
						glVertex3d(piramid[i].index[4].x, piramid[i].index[4].y, piramid[i].index[4].z);
						glEnd();
...
void Timerfunction(int value) {
	...
	moon.rot.degree += 0.1;
	moon.pos.z = 50 * cos(moon.rot.degree * PI + 180);
	moon.pos.x = 50 * sin(moon.rot.degree * PI + 180);

	lamp1.move.degree += 0.1;
	lamp2.move.degree += 0.1;

	lamp1.pos[0] = 100 * cos(lamp1.move.degree * PI) + moon.pos.x;
	lamp1.pos[2] = 100 * sin(lamp1.move.degree * PI) + +moon.pos.z;

	lamp2.pos[0] = 100 * cos(lamp2.move.degree * PI + 180) + moon.pos.x;
	lamp2.pos[2] = 100 * sin(lamp2.move.degree * PI + 180) + moon.pos.z;
