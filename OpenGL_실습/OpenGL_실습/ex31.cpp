...
GLvoid drawScene(GLvoid)
{
	...
	glPushMatrix();//-----------------------------------
	{
		...
		glPushMatrix();//-------------그리기 입력--------------------------
		{
			...
			glDisable(GL_LIGHT0); 	glDisable(GL_LIGHT1);//구에서만 조명 사용하기
			...
			glPushMatrix();
			{
				...
				glTranslated(lightPos2[0], lightPos2[1], lightPos2[2]);
				glutSolidCone(5, 10, 8, 8);
			}
			glPopMatrix();
...
void Keyboard(unsigned char key, int x, int y) {
	...
	case 'n':
		count += 0.1;
		lightPos[0] = 100 * cos(count * PI);
		lightPos[2] = 100 * sin(count * PI);
		break;
